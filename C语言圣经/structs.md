...
1. 结构体把不同成员组织成一个对象，成员通过点运算符访问。
2. 结构体指针使用 ->，它与 (*pointer).member 等价。
3. 结构体可能包含对齐填充，不能假定 sizeof 等于成员大小之和。
4. 文件操作遵循打开、检查、读写、关闭的顺序。
5. stdout、stdin 和 stderr 是程序启动时提供的三个标准流。
6. fflush 主要用于刷新输出流，不要把 fflush(stdin) 当作可移植的清空输入方法。
7. fseek、ftell 和 rewind 用于操作文件位置指示器。
8. 联合体成员共享存储，通常需要枚举标签记录当前有效成员。
9. 枚举常量可以替代魔法数字，但外部输入仍然需要校验。
