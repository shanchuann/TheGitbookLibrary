...
> **学习路径**：结构体把不同类型的成员组织成一个对象，文件和联合体则进一步展示对象布局与表示的边界。掌握这些构造后，下一章用关键字回顾类型、控制流和存储类别背后的语言规则。
...
结构体字段顺序会影响填充和对齐，但不应为了省几个字节就牺牲接口可读性。跨模块使用的结构体应通过头文件暴露稳定字段；只在一个源文件内部使用的结构体可以采用不透明声明，把布局隐藏起来。
