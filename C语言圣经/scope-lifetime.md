...
> **学习路径**：作用域回答“代码能否看见这个名字”，存储期回答“对象何时存在”，二者共同解释了局部变量、全局变量和动态对象的行为。接下来用运算符把这些对象组织成表达式，再进入指针的地址级访问。
...
阅读一段涉及全局变量、`static` 或动态内存的代码时，可以依次问：名字在哪个作用域可见，对象的存储期何时开始和结束，跨文件引用是否形成外部链接。把这三个问题分开，通常比背诵“栈、堆、全局区”的简化图更可靠。
