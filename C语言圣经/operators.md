...
操作数是参与运算的数据，可以是常量、变量、函数返回值或表达式。运算符是描述操作的符号，例如 `+` 表示加法，`*` 表示乘法。

按操作数数量，运算符分为一元、二元和三元运算符。C 语言只有一个三元运算符：条件运算符 `?:`。

“左值”和“右值”不能简单理解为等号左右两边。可修改左值代表一个可以写入的对象，例如变量、数组元素或指针解引用结果；右值通常表示参与计算或传递的值。
...
![C 语言运算符优先级](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/operator-precedence.png)
...
运算结果是 `int` 类型的 `0` 或 `1`，不是 C++ 的 `bool`。
...
#### 6.1 位移运算符
<<（位左移） >>（位右移）
位移会把操作数的二进制位向左或向右移动指定的位数。移位量必须是非负值，并且小于左操作数提升后类型的位宽。

*<< 位左移* 
操作数1<< 操作数2 : 将操作数1的内存中存储的每一位依次向左移动操作数2位,移出位丢弃,空余位补0,注意有符号数左移会覆盖符号位。

在满足类型和范围条件时，`a << b` 可以近似理解为 `a * 2^b`。有符号数左移溢出可能产生未定义行为。
...
*>>位右移*
操作数1 >> 操作数2：将操作数的二进制位向右移动指定的位数。对无符号数来说，这通常相当于除以 `2^b` 后取整。

对于无符号整数，右移时左侧补 0。负数右移的结果由具体实现决定，不能把它当作跨平台的数学除法。
