...
> **学习路径**：动态内存把对象的生存期从代码块中解放出来，同时把释放责任交给调用者。本章先建立所有权和失败处理，再连接字符串、文件缓冲区以及操作系统的 `brk`/`mmap`。
...
每次申请内存都要记录大小、所有者、释放位置和失败时的清理路径。`realloc` 成功后旧指针失效，失败时旧区域仍然存在；多块资源同时申请时，应按已经成功的部分逆序释放。把这些规则写进接口，而不是只靠调用者记忆。
