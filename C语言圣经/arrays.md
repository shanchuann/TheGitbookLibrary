...
数组的定义决定了数组名、元素类型以及元素个数： `<类型> 数组名[<元素数量>];`

其语法如下: 元素数量在方括号 ( `[]` ) 之间，它必须是大于 0 的整数常量表达式
...
![杨辉三角](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260219121951980.png)

我们通过设定 `arr[i] = 1` 来保证每一层的结尾都为 1，然后判断是否应该向前进行下一步，如果 `i - 1 > 0` 则说明前方仍有数据则进行覆盖若没有则退出二层循环，进行数据的打印，将下一个空间设置为 1，重复判断是否应该从后向前覆盖，按同样的规则继续。
...
![](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260219121918719.png)
...
![二分查找](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260219123637233.png)
...
`rightMovearray()` 和 `leftMovearray()` 实现了数组循环移动的基础函数，分别对应数组右移一位和左移一位的操作，当数组长度小于等于 1 时直接返回，避免无效操作和数组越界问题。

其中右移一位的函数会先暂存数组最后一个元素，防止后续元素移位时被覆盖丢失，再从数组末尾开始倒序遍历，把每个元素依次向后挪动一位，最后把提前暂存的末尾元素放到数组的起始位置，完成所有元素的循环右移；

左移一位的函数则是完全反向的逻辑，先暂存数组的第一个元素，再从数组头部开始正序遍历，让每个元素依次向前挪动一位，最后把暂存的起始元素放到数组的末尾，完成循环左移。

`rightMovearray_K()` 和 `leftMovearray_K()` 在单步移动函数的基础上，实现了数组任意位数的循环移动，通过循环调用对应方向的单步移动函数，重复执行 K 次来完成批量移位，同时做了两处关键的优化和容错处理。

首先会通过 `k % n` 的取模运算计算出有效移动位数，因为对于长度为 n 的数组来说，移动 n 位就相当于回到了初始状态，没有任何变化，比如长度为 10 的数组移动 13 位，实际有效移位只有 3 位，取模操作可以避免大量重复的无效循环；

如果取模后的结果为 0，说明移动后数组和原数组完全一致，函数会直接返回，不执行多余的操作。
...
这是一种更优雅的数组移动方案，它通过三次局部反转来实现整体循环移动，时间复杂度优化为 O(n)，空间复杂度仍为 O(1)。这种方法避免了重复遍历数组的低效操作，仅通过有限次的元素交换就能完成目标，在处理大规模数组时性能优势尤为明显，既不需要额外的辅助数组占用内存，也能保证线性的时间效率。

对于长度为 n 的数组，右移 k 位可以拆解为三步：

* 首先翻转整个数组，让原本位于数组末尾的 k 个元素 “移动” 到数组的前半部分，但此时这两部分元素的内部顺序是颠倒的；
* 接下来翻转前 k 个元素，把这部分元素的顺序调整回来，让它们恢复原本的相对位置；
* 最后翻转后 n-k 个元素，同样恢复这部分元素的原有顺序，经过这三次局部反转，就能实现数组整体右移 k 位的效果，整个过程只需要遍历数组两次（三次翻转的总操作次数为 n 次）。
...
![三次翻转法](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260219151101499.png)
...
C 语言没有独立的字符串类型。通常所说的“字符串”，是指一段以空字符 `'\0'` 结尾的字符数组。

`strlen()` 只能处理满足这个条件的字符数组。它会从首元素开始逐个读取，直到遇到 `'\0'`，并返回终止符之前的字符数量。
...
int main(void) {
 ...
    char strb[] = {'h', 'e', 'l', 'l', 'o'};
​
    printf("sizeof(stra) = %zu\n", sizeof stra);
    printf("strlen(stra) = %zu\n", strlen(stra));
​
    /*
     * strb 没有 '\0'，不能传给 strlen、printf("%s") 等字符串函数。
     * strlen(strb);  // 未定义行为
     */
​
    return 0;
...
其中前五个字节是可见字符，最后一个字节是字符串结束标志。

```c
char stra[] = {"hello"};
```

虽然使用了花括号，但 `"hello"` 仍然是字符串字面量，初始化时会自动包含结尾的 `'\0'`。因此：
...
`sizeof` 计算数组占用的总字节数，`strlen` 计算 `'\0'` 之前的字符数。两者含义不同。
...
数组末尾没有 `'\0'`，所以它不是一个 C 字符串。下面的调用都是错误的：
...
注意，`strlen` 不知道数组的真实长度，也不会检查数组边界。它会继续读取 `strb` 后面的内存，直到偶然读到一个 `'\0'`。结果可能是：

* 输出一个看似正常的数字；
* 输出一个很大的错误数字；
* 访问无效内存并崩溃；
* 在不同编译器、不同优化级别或不同运行环境下得到不同结果。

这不是编译器差异导致的正常行为，而是程序违反了字符串函数的前置条件。

### 正确的字符数组初始化

如果数组需要作为字符串使用，必须为结尾的 `'\0'` 预留空间：

```c
char strb[] = {'h', 'e', 'l', 'l', 'o', '\0'};

printf("sizeof(strb) = %zu\n", sizeof strb);
printf("strlen(strb) = %zu\n", strlen(strb));
printf("%s\n", strb);
```
...
```c
char strb[5] = "hello"; // 错误：还需要一个字节保存 '\0'
```
...
```c
char a[] = "hello";                         // 字符串，包含 '\0'
char b[] = {'h', 'e', 'l', 'l', 'o'};       // 字符数组，不是字符串
char c[] = {'h', 'e', 'l', 'l', 'o', '\0'}; // 字符串
```
...
`b` 不能直接传给这些函数，因为它没有终止符。

### 函数参数中的长度问题

数组传给函数后，通常会退化为指向首元素的指针。函数本身无法知道数组有多少个元素：

```c
#include <stddef.h>

void print_chars(const char chars[], size_t count) {
...
调用时应该将数组的长度作为参数一并传递给函数：

```c
char chars[] = {'h', 'e', 'l', 'l', 'o'};
print_chars(chars, sizeof chars / sizeof chars[0]);
```

如果参数代表的是字符串，则函数应要求调用者提供以 `'\0'` 结尾的数组：
...
这里的 `text` 必须指向一个有效的、以 `'\0'` 结尾的字符序列。

#### 长度类型应使用 `size_t`

`strlen` 的返回类型是 `size_t`，不是 `int`。因此推荐使用 `%zu` 输出：

```c
size_t length = strlen(stra);
printf("length = %zu\n", length);
```
...
在很大的字符串上，`size_t` 转换为 `int` 可能发生截断。
...
![二维数组](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260215161140080.png)

二维数组在本质上由多个一维数组构成(每个一维数的大小必须相同)。如定义 `int ar[3][4]` 的二维数组，它是由 3 个一维数组组成，每个一维数组的大小是 4 个整型元素。

数组可以只对部分元素赋值，未赋值的元素自动取 0 值。
...
<figure><img src="https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260215163510987.png" alt=""><figcaption></figcaption></figure>

在 C 语言中，二维数组存放形式是按行优先存储，也就是物理表现形式如图：

![行优先存储](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260215195306920.png)

<table><thead><tr><th width="103.39996337890625">元素</th><th width="146">起始地址</th><th>占用的字节地址范围</th><th>十进制地址范围</th></tr></thead><tbody><tr><td><code>ar[0][0]</code></td><td>0x00b3fbac</td><td>0x00b3fbac ~ 0x00b3fabf</td><td>11786156 ~ 11786159</td></tr><tr><td><code>ar[0][1]</code></td><td>0x00b3fbb0</td><td>0x00b3fbb0 ~ 0x00b3fbb3</td><td>11786160 ~ 11786163</td></tr><tr><td><code>ar[0][2]</code></td><td>0x00b3fbb4</td><td>0x00b3fbb4 ~ 0x00b3fbb7</td><td>11786164 ~ 11786167</td></tr><tr><td><code>ar[0][3]</code></td><td>0x00b3fbb8</td><td>0x00b3fbb8 ~ 0x00b3fbbb</td><td>11786168 ~ 11786171</td></tr><tr><td><code>ar[1][0]</code></td><td>0x00b3fbbc</td><td>0x00b3fbbc ~ 0x00b3fbbf</td><td>11786172 ~ 11786175</td></tr></tbody></table>
...
这里的 `printf()` 会打印哪两个数字呢？

![](https://raw.githubusercontent.com/shanchuann/TheGitbookLibrary/main/C%E8%AF%AD%E8%A8%80%E5%9C%A3%E7%BB%8F/.gitbook/assets/book-images/typora/image-20260215210435109.png)

可以看见，本书将下标访问转化为指针访问的形式之后，第一个数字将输出 8。

`dx[1]` 等同于 `*(dx + 1)`，也就是 **p** 指向的是 3 的地址，而 `p[3]` 就是 `*(p + 3)`，**p** 是一个指向整型的指针，因此 **p** 将偏移 3 个整型的偏移量指向6。
...
这是两个不同的概念，1 是常规的二维数组，在栈区开辟 30 字节的空间后进行部分初始化，其余补 0，2 存储的是指向字符串常量的指针，只开辟 12 字节大小，为了保证能编译通过，最好加上 **const** 来保证不能改变其内容。

这里需要注意，数组传给函数后通常退化为指向首元素的指针，函数无法从指针本身得知元素数量。因此接口应同时传入 `size_t count`，或者把长度放进结构体。二维数组还必须保留第二维的大小，例如 `int matrix[][4]`，否则编译器无法计算下一行的地址。
